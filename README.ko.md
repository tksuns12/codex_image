...
quickstart 이후 스킬 유지보수가 필요할 때만 이 섹션을 사용하세요.

- 상호작용 설치는 `codex-image skill install`로 시작하며 선택은 `Space`, 확정은 `Enter`를 사용합니다.
- 수동 수정/변조된 설치 대상을 덮어쓰거나 삭제하려면 명시적으로 `--force`가 필요합니다.
- 지원 도구/경로 기준표: [docs/skill-paths.md](docs/skill-paths.md)
...
바이너리 업데이트는 GitHub Release artifacts를 사용합니다. 필요 시:
...
- `CODEX_IMAGE_CODEX_BIN`은 선택 사항이며 사용할 Codex 실행 파일 경로를 지정합니다.

## 검증 스크립트

로컬 설치/계약 확인(라이브 생성 없음):
...
실제 Codex 기반 생성까지 확인하려면 런북을 먼저 읽고 실행하세요: [docs/uat-live-smoke.md](docs/uat-live-smoke.md)
